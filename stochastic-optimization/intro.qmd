...
We can view this as a tree, shown in @fig-tree2, where the root node corresponds to moves of nature to generate $(S,W)$, nodes at depth 1 correspond to the moves of the agent to generate $A$.
...
column corresponds to the value of $w$. For example $\PR(S = 1, Y = 1, W = 3) = 0$. Note that the joint distribution of $(S,Y,W)$ is such that the marginal distribution of $(S,W)$ is the same as the distribution on $(S,W)$ given in the previous exercise (@exr-stoc-optim-computing).
