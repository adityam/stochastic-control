...
In the stochastic optimization problems considered in the [previous chapter](intro.qmd), the decision maker does not observe any data before making a decision. In many situations, the decision maker does observe some data before making a decision. In this chapter, we present various models for capturing such situations.

:::{#exm-simple-card-game}
#### A simple card game
Consider a gambler playing a stylized version of card game played with 
a deck of $4$ cards: $\{1,2,3,4\}$. A dealer deals two cards: one to himself and one to the gambler. The gambler looks at his card, and can decide to take one of two actions: fold or challenge. If the gambler folds, the game is over; if he challenges and has a higher card than the dealer, he wins \\$1; if he challenges but has a lower card than the dealer, he loses \\$1. What is the optimal policy?
:::

We can model the above example as follows. Let $W \in \ALPHABET W$ denote the state of the world. The decision maker makes an observation $S \in \ALPHABET S$ and then chooses an action $A \in \ALPHABET A$ as function of his observations according to a _decision rule_ or _policy_ $π \colon \ALPHABET S \to \ALPHABET A$, i.e., $$ A = π(S). $$

Upon choosing the action $A$, the decision maker incurs a cost $c(S,A,W). We assume that the _primitive random variables_ $(W,S)$ are defined on a common probability space and have a known joint distribution. Assume that the decision maker is risk neutral and, therefore, wants to minimize $\EXP[ c(S, π(S), W)]$, where the expectation is taken with respect to the joint probability distribution of $(W,S)$. 
...
Define $J(π) = \EXP[ c(S, Y, π(S)) ]$. Then, Problem \eqref{eq:obs} is conceptually the same as the stochastic optimization problems considered in the previous section but with one difference: the basic stochastic optimization problem is a **parameter optimization problem** where the minimization is over a parameter $a$, while Problem \eqref{eq:obs} is a **functional optimization problem** where the minimization is over a function $π$. 
...
They then ask what action $a$ should they take to minimize the expected
...
(Problem \eqref{eq:cond-1}), one for each possible value of $s$. 
...
We can model this as a stochastic optimization problem. Let $W$ denote the cards dealt, $S$ denote gambler's card, and $A=0$ denote the fold action and $A=1$ denote the challenge action. Then:
$$
\ALPHABET W = \{(1,2), (1,3), (1,4), (2,1), (2,3), (2,4), (3,1), (3,2), (3,4), (4,1), (4,2), (4,3) \},
$$
where the first index denotes the card dealt to the dealer and the second denotes the card dealt to the gambler. We will assume that each of these outcomes is equally likely. 
...
There are $|\ALPHABET A|^{|\ALPHABET S|} = 2^4 = 16$ policies. The performance of a policy $π$ is:
$$
...
We can view this as a tree, shown in @fig-tree1, where the root node corresponds to moves of nature (denoted by $c$ for chance) to generate $S$, nodes at depth 1 correspond to the moves of the agent (denoted by $d$ for decision maker) to generate $A$, and nodes at depth 2 correspond to moves of nature to generate $W$.[^simplify]
...
We can view this as a tree, shown in @fig-tree2, where the root node corresponds to moves of nature to generate $(W,S)$, nodes at depth 1 correspond to the moves of the agent to generate $A$.
...
The difference between the two approaches is conceptual. For the one-step optimization problem, the actual mathematical calculations done in both formulations are the same. When we go to multi-stage optimization problem, we will see that perfect and imperfect observation models behave very differently. 
