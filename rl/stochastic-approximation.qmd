...
obtain the value $f(θ)$, but it may be possible to conduct an experiment (or run a simulation) to
get the value of $f(θ)$ with noise. In such settings one can try to estimate $f(θ)$ for a particular value of $θ$ by running the simulation a large number of times and averaging them to get a better estimate of $f(θ)$ and then use this estimate in one of the iterative schemes to find the root of the function.
However, such a procedure will be inefficient because we require many simulations to get a good value of $f(θ_t)$ at iteration $t$, even when $θ_t$ is far away from $θ^*$. It makes sense to use the simulations to find better values of $θ_t$ rather than improving the estimate of $f(θ_t)$. This is exactly what stochastic approximation does.

Stochastic approximation refers to recursive algorithms of the form
...
:::{#exr-SA-cubic}
Consider the standard stochastic approximation iteration \eqref{eq:SA} with
$$
  f(θ) = -θ^3.
$$

a. Is the ODE $\dot θ = f(θ)$ globally asymptotically stable? (_Hint:_ Show that $V(θ) = θ^2$ is a valid Lyapunov function).

b. Is origin globally asymptotically stable equilibrium point of the scaled ODE $\dot θ = f_{∞}(θ)$. where $f_{∞}(θ)$ is defined as in @thm-borkar-meyn. 

c. Is $\IP{\GRAD V(θ)}{f(θ)} \le 0$ for all $θ$?

d. A simulation of \eqref{eq:SA} for the above choice of $f$ and $\{ξ_t\}_{t \ge 1}$ being i.i.d. noise with $ξ_t \sim \text{Unif}[-1,1]$.for different stating conditions is shown in @fig-cubic. Explain the behavior of the iterates.
    ...
    θ = zeros(T,M)
    for m in 1:M
        θ[1,m] = 2*mod((m/M + rand()/M),1) - 1
        for t in 1:T-1
            θ[t+1,m] = θ[t,m] + α[t]*f(θ[t,m]) + α[t]*rand(Uniform(-1,1))
        end
    end
    ```
...
```{ojs}
//| label: fig-cubic
//| fig-cap: Multiple runs for $f(θ) = -θ^3$
//| output: asis
...
Plot.plot({
  ...
  marks: [
    ...
    Plot.ruleY([0]),
    // Data
    Plot.line(cubic, {x: "time", y: "value", z: "run", stroke: "#9ecae1" })
  ]}
)
```
:::
...
d. A simulation of \eqref{eq:SA} for the above choice of $f$ and $\{ξ_t\}_{t \ge 1}$ being i.i.d. noise with $ξ_t \sim \text{Unif}[-1,1]$.for different stating conditions is shown in @fig-saturation. Explain the behavior of the iterates.
    ...
    ojs_define(saturation = df)
    ```

```{ojs}
//| label: fig-saturation
//| fig-cap: Multiple runs for $f(θ) = -θ/\sqrt{1+θ^2}$
//| output: asis
...
Plot.plot({
  ...
  marks: [
    ...
    Plot.ruleY([0]),
    // Data
    Plot.line(saturation, {x: "time", y: "value", z: "run", stroke: "#9ecae1" })
  ]}
)
```
:::
