...
$$
  V(θ + η) \le V(θ) + \langle \GRAD V(θ), η \rangle + M \NORM{η}^2_2.
$$
...
$$
  \left(\frac{d g}{d λ}\right)(λ) = \langle η, \GRAD V(θ + λ η) \rangle.
$$
Therefore, we have
\begin{align*}
  V(θ + η) - V(θ) &= W(1) - W(0) 
  = \int_{0}^1 \left(\frac{d g}{d λ}\right)(λ) d λ \\
  &= \int_{0}^{1} \langle η, \GRAD V(θ + λ η) \rangle d λ \\
  &\le \int_{0}^{1} \langle η, \GRAD V(θ) \rangle d λ 
  + \biggl| \int_{0}^{1} \langle η, \GRAD V(θ + λ η) - \GRAD V(θ) \rangle d λ \biggr| \\
  &\stackrel{(a)}\le 
   \langle η, \GRAD V(θ) \rangle 
  +  \int_{0}^{1} \NORM{η}_2 \NORM{\GRAD V(θ + λ η) - \GRAD V(θ)}_2 d λ \\
  &\stackrel{(b)}\le 
   \langle η, \GRAD V(θ) \rangle
  + \NORM{η}_2  \int_{0}^{1} 2 M λ \NORM{η}_{2} d λ \\
  &= 
   \langle η, \GRAD V(θ) \rangle + M \NORM{λ}^2
\end{align*}
