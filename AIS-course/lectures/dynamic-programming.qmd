...
In [Lecture 1](static-optimization.qmd) we considered static stochastic optimization: choose one action, possibly after seeing an observation. Now the decision maker acts repeatedly over time. To fix ideas, we start from a simple multi-stage version of the newsvendor problem.
...
Suppose that the primitive random variables $\{W_0,\dots,W_T\}$ are **independent** (but not necessarily identically distributed). Then the conditional law of $W_t$ given $H_t = h_t$ does not depend on the past:
$$
...
Initialize $V^\star_{T+1}(\cdot) \equiv 0$. For $t = T, \dots, 1$, and under independence of the primitive random variables,
\begin{align}
  Q^\star_t(h, a)
...
\begin{equation}\label{eq:state-additive-cost}
...
In a state space model \eqref{eq:state-space-model} with additive cost \eqref{eq:state-additive-cost} and independent primitive random variables, the optimal action-value functions depend on $h_t$ only through the current state $s_t$:
\begin{equation*}
