...
- **Event driven optimization**
    ...
    In the models discussed in class, decisions need to be made at every time. One way to reduce the computational complexity is to consider the settting where decisions are made only when certain events occur. An overview of this approach is presented in 
...
- **Periodic MDPs**
    ...
    In class, we restricted attention to time-homogeneous models. In practice, especially in applications like power systems, the dynamics are often periodic. A potential term project on this topic will include an overview of the main ideas of periodic MDPs introduded in
...
- **Risk-sensitive MDPs**
    ...
    In the course, we primairly restricted to risk neutral setup. In many settings, we are interested in [risk sensitive utility](risk-sensitive/risk-sensitive-utility.qmd) function. A potential term project could be to review the formulation of risk sensitive MDPs (links to be added).
...
- **Establishing properties beyond monotonicity**
    ...
    A related result is the theory of comparitive statistics, which establishes monotonicity in any model parameter (including discount factors)
...
- **Monotonicity for multi-dimensional MDPs**
    ...
    In class, when discussing monotonicity results, we assumed that the state on one-dimensional. The general theory for multi-dimensional setting is presented in the following
...
- **Multi-armed bandits**
    ...
    Multi-armed bandits[^MAB] are a special class of resouce allocation problems which have a low complexity solution. In general, scheduling problems can be modeled as MDPs, but the state space of such problems in exponential in the number of alternatives. This is sometimes referred to as the _curse of dimensionality_. A conceptual breakthrough in such problem was achieved when Gittins showed that 
    ...
    A potential term project on this topic is to review the basic result of @Gittins1979. The proof in @Gittins1979 uses [interchange argument](stochastic-optimization/interchange.qmd), which we did not cover in the course. So the term project could the method of interchange arguments, and then explain Gittins result using that. 

    Another alternative is to present the dynamic programming proof of Gittins result, which was given presented in
...
- **Policy gradient algorithms**
    ...
    This is a long paper and the project; a term project on this paper can focus on the tabular setting.
