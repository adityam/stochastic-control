...
We now solve the tracking problem. Conisder the same parameters as before. We consider tracking a "square-wave" tracking signal. The output (i.e., position) and the input (i.e., force) for a $T = 60\,\text{s}$ simulation of the system are shown in @fig-lqr-tracking.
...
```{ojs}
viewof Ro = Inputs.range([0.1, 2.5], {label: "R", step: 0.1, value: 0.1 })
```
