...
Now we consider the tracking problem. For simplicity, we will consider state tracking rather than output tracking. That is, we are given a pre-specified trajectory $\{x^\circ_t\}_{t=1}^T$ and the objective is to minimize a per-step cost given by
  $$ \begin{align*}
    c_t(x_t, u_t) &= (x_t - x^\circ_t)^\TRANS Q (x_t - x^\circ_t) + u_t^\TRANS R u_t
    \\
    \text{and}\quad
...
$$
Let $\bar K_t \eqqcolon \MATRIX{K_t & u^\circ_t}$. Then, the optimal control is of the form:
$$
  u_t = - \bar K_t z_t = - K_t x_t - u^\circ_t 
$$

It can be shown that the gain $K_t$ s the same gain as $\LQR_T(A,B,Q,R;Q_T)$. Thus, qualitatively, the tracking control input is equal to the regulation control input plus a correction term. 
...
for pkg in ["IJulia", "Distributions", "DataFrames", "JSON"]
    Pkg.add(pkg)
end
...
function riccati_update(P::AbstractMatrix, sys:: SSModel, cost :: SSCost)
  A, B, Q, R, S = sys.A, sys.B, cost.Q, cost.R, cost.S
  Δ = R  + B' * P * B
  Λ = S' .+ B' * P * A
  K = Δ \ Λ
  P_new = Q + A' * P * A - Λ' * K
...
function LQR(sys :: SSModel, cost :: SSCost, T :: Int)
   P = [ zeros(size(sys.A))   for t in 1:T ]
   K = [ zeros(size(sys.B'))  for t in 1:T-1 ]

   P[T] = cost.QT
   for t in T-1:-1:1
      P[t], K[t] = riccati_update(P[t+1], sys, cost)
   end
   return (P,K)
end

function lsim(x_init, sys, K, T, σ²)
    W = Normal(0, σ²)
    ...
    for t = 1:T-1
        u[t] = -K[t]*x[t] 
        x[t+1] = sys.A * x[t] + sys.B * u[t] + sys.B * w[t]
    end
    return (x,u)
end
...
for ρ in R
    sys = SSModel(A,B)
    cost = SSCost(Q,ρ,S,QT)

    P, K = LQR(sys, cost, T)
    x, u = lsim([1;0], sys, K, T, σ2)
...
_Hint_: There are two ways to solve this problem. An algebriac method using where one can argue that the Riccati gain $P$ is diagonal and a simpler method that uses certainty equivalence. 
:::
...
Deterministic LQR model for continuous time systems was proposed in @Kalman1960b. The proof idea of completion of squares is due to @Astrom1970 but we loosely follow the proof outlines adapted from @Afshari2023.
