...
$$
  ...
  R = ρ
$$
We simulate this system for $T = 10\,\text{sec}$ with noise covariance $σ^2 = 0.05$. The output (i.e., position) and the input (i.e., force) are shown in @fig-lqr-integrator. Note that, as expected, as the cost of applying control increases, less force is applied and it takes longer for the position to become close to zero. 
