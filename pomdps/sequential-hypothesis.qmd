...
observation at time $t$. The DM wants to differentiate between two hypotheses:
\begin{gather*}
  h_0 : Y_t \sim f_0 \\
  h_1 : Y_t \sim f_1
\end{gather*}
...
h_1)$ indicates that the DM declares everything is okay when there is an anomaly.
...
1. For general POMDPs we were using $h_t$ to denote histories. In this section, we are following the standard notation from hypothesis testing and using $h_0$ and $h_1$ to denote the hypotheses.

2. This is an [optimal stopping](../mdps/optimal-stopping.qmd) problem. As we saw earlier, for MDPs, optimal stopping is a special case of general MDPs. The same is true for POMDPs as well. 

3. The above model is a POMDP where the unobserved state is the true hypothesis $H$. Note that the true hypothesis $H$ does not evolve with time.
...
programming decomposition. Recall that the belief state is a two-dimensional pdf
where
...
:   * We are only conditioning on $Y_{1:t}$ and not adding $A_{1:t-1}$ in the
      ...
      in optimal stopping problems where we are only defining the state for the
      case when the stopping decision hasn't been taken so far and all
      previous actions are continue. Taking a continue action does not affect
    ...
    * It is possible to exploit the fact that $b_t = [p_t, 1 - p_t]^T$ and
      ...
      the case of non-binary hypotheses.
...
The above functions satisfy the following properties:

* $L_i(b)$ is linear in $b$.
* $V_t(b)$ and $W_t(b)$ are concave in $b$.
...
monotonicity in time then follows from @exr-monotonicity-in-time. Since $V_t$ is monotone in time, it implies that
...
$A_t(h_i)$ is the set of $b$ where one linear function of $b$ is less than
...
The monotonicity of $D_t(h_i)$ in time follows from the monotonicity of $W_t$
in time.
:::

:::{#thm-hypothesis-corner}
Suppose the stopping costs satisfy the following:
...
Note that $L_i(e_0) = \ell(h_i, h_0)$ and $L_i(e_1) = \ell(h_i, h_1)$.
...
we formulated the problem for binary hypotheses, all the steps of the proof
...
<figure>
  <img src="images/general-stopping.png" 
       ...
       width="300" />
  <figcaption>Stopping regions for multiple hypotheses</figcaption>
</figure>
:::

For binary hypotheses, we can present a more concrete characterization of the
optimal policy. Note that the two-dimensional simplex is equivalent to the
interval $[0,1]$. In particular, any $b \in Δ^2$ is equal to $[p, 1-p]$, where
$p \in [0,1]$. Now define:
...
The above property is more simply stated in terms of the likelihood
ratio. In particular, define $λ_t = b_t(h_0)/b_t(h_1) = p_t/(1 - p_t)$. Then, we
have the following:
...
$$ \hat π_t(λ) = \begin{cases}
   ...
   \mathsf{C}, & \text{if } α_t/(1 - α_t) < λ < β_t/(1 - β_t) \\
   h_0, & \text{if } β_t/(1 - β_t) \le λ.
  \end{cases} $$
:::
...
with the standard [:likelihood ratio test][LRT] in hypothesis testing.
...
where
$$ W(b) = c + \int_{y} [ b(h_0)f_0(y) + b(h_1)f_1(y)] V(ψ(b,y)) dy. $$
...
infinite horizon model, we can get an upper bound on the expected number of
...
$$
  J(π) = \EXP[ c τ + \ell(A_\tau, H) \mid B_1 = b ].
$$
Note that $\ell(A_\tau, H) \ge 0$. Therefore, the performance of the optimal
policy is lower bounded by
$$
  J^* \ge c\, \EXP^{π^*}[  τ \mid B_1 = b] .
$$
...
As in the model discussed above, there are two hypotheses $h_0$ and $h_1$.
...
b. Show that the optimal control law has a threshold property, similar to
   the threshold property for the model described above.
:::
...
b. Now assume that the policy $π$ is of a threshold form with thresholds
  ...
  \PR(H = h_0 | Y_{1:t})$ is a Markov chain which starts at a state $p
  \in (a,b)$ and stops the first time $p_t$ goes below $a$ or above $b$.
    ...
    Suppose we discretize the state space $[0, 1]$ into $n+1$ grid
    points $\ALPHABET D_n = \{0, \frac1n, \dots, 1\}$. Assume that $p$,
    $a$, and $b$ lie on this discrete grid. Discretize $p_t$ to the
...
For more details on sequential hypothesis testing, including an approximate
method to determine the thresholds, see @Wald1945. The optimality of the sequential
