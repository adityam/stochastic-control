...
[model of MDPs described earlier][MDP].
...
_primitive random variables_ $(S_1, W_1, \dots, W_T$, $N_1, \dots, N_T)$ are
...
Note that the only difference from the MDP model is that the decision maker observes
$Y_t$ instead of $S_t$. Apart from this, the other modeling assumptions are
the same. So, the conceptual difficulties of the model are the same as those of
MDPs:
...
with time. Therefore, the number of possible control laws at time $t$ is
increasing exponentially with time. How can we efficiently search
...
Our first step toward developing an efficient dynamic programming decomposition is to
...
\begin{align*}
  J_t(h_t; π) &= \EXP^π\biggl[ \sum_{τ=t}^T c_τ(S_τ, π_τ(H_τ)) \biggm|
    H_t = h_t \biggr] \\
    &\stackrel{(a)}= \EXP^π \biggl[ c_t(S_t, π_t(h_t)) + \EXP^π\biggl[ 
    \sum_{τ=t+1}^T c_τ(S_τ, π_τ(H_τ)) \biggm| H_{t+1} \biggr] \biggm| 
    H_t = h_t \biggr]  \\
    &= \EXP^π[ c_t(S_t, π_t(h_t)) + J_{t+1}(H_{t+1}; π) \mid H_t = h_t ],
\end{align*}
...
\in \{T, \dots, 1\}$, 
$$
J_t(h_t; π) = \EXP^π [ c_t(S_t, π_t(h_t)) + J_{t+1}(H_{t+1}; π) \mid
  H_t = h_t ].
$$
...
with equality at $t$ if and only if the _future_ strategy $π_{t:T}$
...
obtained using the dynamic program of @thm-pomdp-history-dp is optimal. The proof of
...
\begin{align*}
  ...
  &\stackrel{(a)}= \min_{a_t \in \ALPHABET A}
   \EXP^π[ c_t(S_t, A_t) + V_{t+1}(H_{t+1}) \mid
  H_t = h_t, A_t = a_t ] 
  \\
  &\stackrel{(b)}\le 
   \EXP^π[ c_t(S_t, A_t) + V_{t+1}(H_{t+1}) \mid
  ...
  \\
  &\stackrel{(c)}\le
   \EXP^π[ c_t(S_t, A_t) + J_{t+1}(H_{t+1}; π) \mid
  H_t = h_t, A_t = π_t(h_t)] 
  \\
  &= J_t(h_t; π).
\end{align*}
...
hypothesis. We have the equality at step $(b)$ iff $π_t$ satisfies the
...
_information state_ if $Z_t$ is a function of $H_t$ (which we denote by $Z_t =
...
\begin{align*}
  ...
  \mid H_t = h_t, A_t = a_t) 
  \\
  &\stackrel{(b)}= \sum_{y_{t+1} \in \ALPHABET Y} \IND\{ ψ_t(φ_t(h_t), y_{t+1}, a_t) \in B \}
  \PR(Y_{t+1} = y_{t+1} \mid H_t = h_t, A_t = a_t) 
  \\
  &\stackrel{(c)}= \sum_{y_{t+1} \in \ALPHABET Y} \IND\{ ψ_t(φ_t(h_t), y_{t+1}, a_t) \in B \}
...
We start by defining the _belief state_ $b_t \in Δ(\ALPHABET
...
:::{.callout-important}
#### Significance of policy independence of conditional independence

This is an extremely important result, with wide-ranging implications in stochastic control. For a general discussion of this point, see @Witsenhausen1975.
:::
...
\\eqref{eq:belief}. Observe that the terms of the form $\IND\{ a_τ =
π_τ(y_{1:τ}, a_{1:τ-1}) \}$ are common to both the numerator and the denominator
and cancel each other. Thus,
\begin{equation} \label{eq:belief-fn}
  \PR(s_t | y_{1:t}, a_{1:t-1}) = \sum_{s_{1:t-1}}
  \frac{ \prod_{τ=1}^t \PR(s_{τ} \mid s_{τ-1}, a_{τ-1}) \PR(y_{τ} \mid s_{τ}) }
  { \sum_{s'_{1:t}} \prod_{τ=1}^t \PR(s'_{τ} \mid s'_{τ-1}, a_{τ-1}) \PR(y_{τ} \mid s'_{τ}) }.
\end{equation}
...
Now we present three examples of information states. See the
...
information states as well. We present some examples of such information
states below.
:::
...
minimum of PWLC functions is PWLC. Hence, the result holds by the principle of
induction.
:::
...
\begin{multline*}
  ...
  \mid S^1_t = s^1_t, S^2_t = s^2_t, A_t = a_t)
\end{multline*}
and the per-step cost does not depend on $s^2$, i.e., 
$$
c(s^1,s^2,a) = c(s^1,a).
$$
...
Consider a POMDP where the agent observes the observation $Y_t$ with a delay $δ$, i.e., the observation $Y_t$ at time $t$ equals $\ell_t(S_{t-δ}, N_{t-δ})$. Show that $(B_{t-δ}, A_{t-δ:t-1})$ is an information state.
:::
...
Show that if the primitive variables are Gaussian, then the conditional
...
Consider a machine which can be in one of $n$ ordered states where the first
...
a fixed cost). Repairing or replacing the machine effectively resets the machine to the best state. The objective is to identify a maintenance policy to
...
The information state may be viewed as a generalization of the
