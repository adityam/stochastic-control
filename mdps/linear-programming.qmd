...
associate a positive dual variable $ζ(s,a)$ for each constraint consider the
following linear program:[^1]
\begin{gather}
  \min \sum_{s \in \ALPHABET S} \sum_{a \in \ALPHABET A} ζ(s,a) c(s,a)
  \notag \\
  \text{subject to}\quad
  ζ(s,a) - γ\sum_{z \in \ALPHABET S} \sum_{a \in \ALPHABET A}
  P(s | z, a) ζ(z,a) = p(s), \quad \forall s \in \ALPHABET S, \notag \\
  ζ(s,a) \ge 0, \quad \forall s \in \ALPHABET S, a \in \ALPHABET A. 
  \label{eq:dual}
\end{gather}
...
Note that the constraint on the occupancy measure $ζ$ is same as the flow equation derived in the proof of @prp-optimal-Markov-randomized.
...
1. Let $π$ be any possibly randomized history dependent policy. Define the
   _occupancy measure_
    ...
    Then, $ζ_π$ is a feasible solution to the dual LP \\eqref{eq:dual}.

2. Suppose $ζ(s,a)$ is a feasible solution to the dual LP. Then for each $s
   \in \ALPHABET S$ such that $\sum_{a \in \ALPHABET A} ζ(s,a) > 0$, define
   a stationary randomized policy $π$ by
   \begin{equation} \label{eq:policy}
    π(a|s) = π_ζ(a|s) = \frac{ ζ(s,a) } { \displaystyle \sum_{a \in \ALPHABET A} ζ(s,a) }. 
   \end{equation}
   Then, $ζ_{π}(s,a) = ζ(s,a)$ for all $s \in \ALPHABET S$ and $a \in \ALPHABET A$. [This was derived in the proof of @prp-optimal-Markov-randomized].

3. A feasible solution of an LP is called a _basic feasible solution_ if it cannot be expression as a nontrivial convex combination of other feasible solutions. A key property of a basic feasible solution is that when the LP has $k$ constraints, then all basic feasible solutions have at most $k$ positive components. Using this, one can show that if $ζ$ is a basic feasible solution, then the policy $π_{ζ}$ given by \\eqref{eq:policy} is a deterministic policy.

4. Similarly, for any deterministic policy $π$, the occupancy measure $ζ_{π}$ is a basic feasible solution to the dual LP. 


5. If $ζ^*$ is an _optimal_ solution to the dual LP, then $π_{ζ}$ is an optimal policy. 

6. If $ζ^*$ is an _optimal basic_ solution to the dual LP, then $π_{ζ}$ is a deterministic optimal policy.

7. If $π$ is a possibly randomized optimal policy for the MDP, then $ζ_{π}$ is a optimal solution to the dual LP.

8. If $π$ is a deterministic optimal policy for the MDP, then $ζ_{π}$ is a optimal basic solution to the dual LP. 
...
\begin{gather}
  ...
  \notag \\
  \text{subject to}\quad
  ζ(s,a) - γ\sum_{s \in \ALPHABET S} \sum_{a \in \ALPHABET A}
  P(s | z, a) ζ(s,a) = p(s), \quad \forall s \in \ALPHABET S, \notag \\
  \sum_{s \in \ALPHABET S} \sum_{a \in \ALPHABET A} ζ(s,a) d(s,a) \le D \notag \\
  ζ(s,a) \ge 0, \quad \forall s \in \ALPHABET S, a \in \ALPHABET A. 
\end{gather}

If we interpret $ζ(s,a)$ as the occupation measure of any policy, then this
