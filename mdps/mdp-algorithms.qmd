...
1. $V^{(k)} + \underline δ^{(k)} \ONES \le V^* \le V^{(k)} + \bar δ^{(k)} \ONES$.
2. $(\underline δ^{(k)} - \bar δ^{(k)}) \ONES  \le V^{π^{(k)}} - V^* \le (\bar δ^{(k)} -
   \underline δ^{(k)}) \ONES.$
:::  
...
inequality
$$ \max\{ V^{π^{(k)}} - V^* \} \le 
   \max\{ V^{π^{(k)}} - V^{(k)} \} + \max\{ V^{(k)} - V^* \}
   \le (\bar δ^{(k)} - \underline δ^{(k)}) \ONES.
$$
Similarly,
$$ 
  \max\{ V^* - V^{π^{(k)}} \} \le
   \max \{ V^* - V^{(k)} \} + \max\{ V^{(k)} - V^{π^{(k)}} \}
   \le (\bar δ^{(k)} - \underline δ^{(k)}) \ONES.
$$
...
1. An implication of @prp-Bellman-properties is that if the initial estimate $V^{(0)} < V^*$, then $\{V^{(k)}\}_{k \ge 0}$ is a monotone increasing sequence that converges to $V^*$. Similarly, if $V^{(0)} > V^*$, then $\{V^{(k)}\}_{k \ge 0}$ is a monotone decreasing sequence that converges to $V^*$.
