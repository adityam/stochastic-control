...
1.  **Stop**: The process terminates, and the decision maker incurs a _stopping cost_ $d_t(S_t)$.

2.  **Continue**: The decision maker incurs a _continuation cost_ $c_t(S_t)$, and the process evolves to state $S_{t+1}$.

The objective is to determine a stopping time $\tau$ to minimize the total expected cost:
$$J(\tau) := \EXP\bigg[ \sum_{t=1}^{\tau-1} c_t(S_t) + d_\tau(S_\tau) \bigg].$$
...
For any time $t \in \{T-1, \dots, 1\}$:
$$V^*_t(s) = \min \Big\{ \underbrace{d_t(s)}_{\text{Stopping cost}}, \quad \underbrace{c_t(s) + \EXP[ V^*_{t+1}(S_{t+1}) \mid S_t = s]}_{\text{Continuation cost}} \Big\}$$
:::
...
Recall that in the section on [monotone MDPs](monotone-mdps.qmd), we identified sufficient conditions (based on submodularity) under which the optimal policy is monotone. Such a structure is desirable for optimal stopping problems as well, where it takes the form of a **threshold policy**.
...
Formally, we define the _benefit (or advantage) function_ as the expected benefit[^1] of delaying the stopping decision at time $t$, i.e.,
...
Note that, we can write the value function in terms of the benefit function as follows:
\begin{align}
...
We now present an example where we can use @thm-monotone-stopping to determine monotonicity of optimal policies.

:::{#exm-time-to-market}
#### Time-to-Market Model

Consider a firm that decides when to introduce a new product. When the firm introduces the product earlier than the competition, it captures a larger market share. However, an early introduction results in high production costs and low profit margins due to low manufacturing yields. Hence, the firm needs to determine the optimal time to enter the market. Suppose that the total market demand $D$ is deterministic. Let $\{S_t\}_{t \ge 1}$ denote the number of competitors at time $t$. It is assumed that
$$ S_{t+1} = S_t + W_t, $$
...
We will use @thm-monotone-stopping to show that the optimal stopping decision is monotone in the state. 

First observe that $\{S_t\}_{t \ge 1}$ is a monotone process. Now consider the one step look-ahead function
\begin{align*}
...
Note that @thm-monotone-stopping is a sufficient condition for monotonicity and cannot always establish monotonicity. We now present an example where we verify monotonicity from first principles.
...
Find the optimal strategy for the investor.
:::
...
We solve @exm-call-options numerically. Since this is a continuous state model, we are solving using [linear function approximation](continuous-state-spaces.qmd#linear-function-approximation) with Gauss-Hermite quadrature for the expectation and Chebyshev collocation points.
...
module CallOptions_LFA
  ...
  function solve(; p=5.0, σ=1.0, T=20, n=8, K=50, K_eval=500, n_quad=32)
    # State bounds: assume stock price in [0, 2p] (reasonable range)
    s_min, s_max = 0.0, 2.0 * p
    ...
    # Basis functions: Legendre polynomials (all degrees, not just even)
    ...
    for t in 1:T
      if t == T
        ...
        Q_continue_eval = E_eval * α[t+1]
        for k in 1:K_eval
          π_eval[t][k] = Q_exercise_eval[k] >= Q_continue_eval[k] ? 1 : 0
        end
      end
    end
...
ojs_define(call_options_T = T_horizon, call_options_df = call_options_df)
```

```{ojs}
viewof call_options_time = Object.assign(Inputs.range([1, call_options_T], {label: "t", step: 1, value: call_options_T/2}), {style: '--label-width:20px'})
```

```{ojs}
//| label: fig-call-options
//| fig-cap: "Call option: Value function $V_t(s)$ and optimal exercise policy $π_t(s)$ at time $t$"
//| fig-subcap:
//|     - Value function
//|     - Optimal policy (1=exercise, 0=continue)
//| layout-ncol: 2
...
We now prove some qualitative properties that can be inferred from the plots show in @fig-call-options.
...
\end{align*}
By the induction hypothesis the second term is decreasing in $s$. The maximum of a constant and a decreasing function is decreasing in $s$. Thus, $V^*_t(s) - s$ is decreasing in $s$. This completes the induction step. 
:::
...
A driver is traveling along a straight road toward a theatre. Parking spaces are located at integer distances $t \in \{0, 1, \dots, T\}$ from the theatre. As the driver approaches the destination, they encounter these spots in decreasing order of distance.

The availability of a spot is revealed only upon reaching it; each spot is available with probability $p$, independently of the others. If the driver chooses to park at a spot with distance $t$, the cost incurred is $t$. If they reach the theatre without having parked, they incur a penalty cost $C$.

Determine an analytic characterization of the optimal policy to minimize the expected cost.
:::

## Notes {-}

The foundation of optimal stopping lies in Abraham Wald's work on sequential analysis during World War II; his most prominent result demonstrated that the sequential probability ratio test could be significantly more efficient than fixed-sample methods [@Wald1947]. Comprehensive references for the modern theory include @Chow1971, @DeGroot1970, and @Peskir2006

@thm-monotone-stopping is taken from @Oh2016.

The solution of the secretary problem presented here is due to @Lindley1961. For a history of the secretary problem and some generalizations, see @Ferguson1989 and @Freeman1983. An interesting generalization is **Robbins' problem**, which was introduced by Herbert Robbins. See @Bruss2005 for a detailed discussion and history. 

The above model for pricing options was introduced by @Taylor1967.

The optimal parking problem from @exr-optimal-parking is adapted from @MacQueenMiller1960.
