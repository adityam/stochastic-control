...
Under @cnj-power-delay-density, for all $t$, $H_t(y,s)$ is supermodular in $(y,s)$. 
...
Moreover, due to @cnj-power-delay-density, we have that $P_W(y' - y^-)
\le P_W(y' - y^+)$. Thus, 
...
::: {.callout-important }
# Even under @cnj-power-delay-density, we cannot establish the monotonicity of $π^*_t(x,s)$ is $s$.

At a intuitive level, we don't expect the optimal policy to be monotone in the channel state. In particilar, consider consider two states $(x,s_1)$ and $(x, s_2)$ where $s_1 \succeq_s s_2$, i.e., $s_2$ is worse than $s_1$. Since the channel is stochastically monotone, this means that the next state when the current state is $s_2$ will be worse than when the current state is $s_1$. Thus, we might be better off transmitting a larger number of packets in $s_2$ than $s_1$, in anticipation of a very bad channel state in the future. Hence, the optimal policy may to be monotone in the channel state.
...
The mathematical model of power-delay trade-off is taken from @Berry2000, where the monotonicty results were proved using first principles. More detailed characterization of the optimal transmission strategy when the average power or the average delay goes to zero are provided in @Berry2002 and @Berry2013. A related model is presented in @Ding2016. A slight generalization of this model is also considered in @Fu2012 where monotonicty in the queue state is stablished. Generalization to constrained average cost settings are presented in @Djonin2007, @Agarwal2008, and @Zhao2020. Generalizations to multi-user settings are presented in @Djonin2007a and @Goyal2008. 

For a broader overview of power-delay trade offs in wireless communication, see @Berry2012 and @Yeh2012.

The remark after @lem-power-delay-submodular shows the difficulty in establishing monotonicity of optimal policies for a multi-dimensional state space. In fact, sometimes even when monotonicity appears to be intuitively obvious, it may not hold. See @Djonin2007a and @Sayedana2020a for examples. For general discussions on monotonicity for multi-dimensional state spaces, see @Topkis1998 and @Koole2006. As an example of using such general conditions to establish monotonicity, see @Sayedana2020. 
