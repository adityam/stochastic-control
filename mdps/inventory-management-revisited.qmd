...
$$\begin{equation}\label{eq:DP-1}
  ...
  \bigl\{  F(s+a) \bigr\}. 
\end{equation}$$
Recall that while proving the structure of the optimal policy, we had shown that $s^* = \arg\min_{s \in \reals}F(s)$ and 
$$
...
$$
F(s) = \EXP[ c^*(s - W) ] + γ F(s^*), \quad s \le s^*.
$$
Taking $s = s^*$ and rearraning, we get
$$
F(s^*) = \frac{1}{1-γ} \EXP[ c^*(s^*-W) ]
$$
Thus,
\begin{equation}\label{eq:F}
F(s) = \EXP[ c^*(s-W) ] + \frac{γ}{1-γ} \EXP[ c^*(s^* - W) ],
\quad s \le s^*.
\end{equation}
...
#### Parial Proof
From the dynamic program, we know that for any $s \le s^*$, not ordering is worse than ordering $s^* - s$. Thus, $F(s) \ge F(s^*)$. Therefore, from \eqref{eq:F}, we get
$$
\EXP[ c^*(s-W) ] + \frac{γ}{1-γ} \EXP[ c^*(s^* - W) ]
\ge
\EXP[ c^*(s^*-W) ] + \frac{γ}{1-γ} \EXP[ c^*(s^* - W) ]
$$
or simply
\begin{equation}\label{eq:ineq-1}
  \EXP[ c^*(s-W) ] \ge \EXP[ c^*(s^*-W) ], \quad s \le s^*.
\end{equation}
...
\begin{equation}\label{eq:ineq-2}
  \EXP[ c^*(s-W) ] \ge \EXP[ c^*(s^*-W) ], \quad s \ge s^*.
\end{equation}
