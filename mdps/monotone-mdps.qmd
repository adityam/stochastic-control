...
b. Show that the value function $V_t(s)$ is convex in~$s$ and the action-value function $Q_t(s,a)$ is submodular in $(s,a)$. Using this, show that the optimal policy has a threshold structure, i.e., there exist a sequence of threshold $σ_t \in \integers_{\ge 0}$, such that the optimal decision at time $t$ is to reject the batch if $S_t \ge σ_t$. 

c. Argue that $σ_t$ is a decreasing sequence. 
