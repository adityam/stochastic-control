...
a. Show that the optimal value functions are weakly increasing and convex in the state.

b. Show that the optimal policy has a threshold structure, i.e., there exist a sequence of threshold $σ_t \in \integers_{\ge 0}$, such that the optimal decision at time $t$ is to reject the batch if $S_t \ge σ_t$. 
