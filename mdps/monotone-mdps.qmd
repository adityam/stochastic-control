...
$$ 
  V^*_t(s_1) = Q^*_t(s_1, a_1^*) \stackrel{(a)}\ge Q^*_t(s_2, a_1^*) \stackrel{(b)}\ge V^*_t(s_2),
$$
where $(a)$ follows because $Q^*_t(\cdot, a_1^*)$ is weakly increasing and $(b)$
...
Let $π^*_t(s) = \max\{ a^* \in \arg \min_{a \in \ALPHABET A} Q^*_t(s,a) \}$. Then, $π^*_t(s)$ is weakly increasing in $s$.
:::
...
Let $π^*_t(s) = \min\{ a^* \in \arg \min_{a \in \ALPHABET A} Q^*_t(s,a) \}$. Then, $π^*_t(s)$ is weakly decreasing in $s$.
:::

:::{.callout-note collapse="true"}
#### Proof 
Conditions (C1) and (C2) imply that the value function $V^*_{t+1}(s)$ is weakly increasing. Therefore, condition (C4) implies that $Q^*_t(s,a)$ is supermodular in $(s,a)$. Therefore, the arg min is weakly decreasing in $s$.
:::
...
idea is as follows. Suppose $V^*_{t+1}(\cdot)$ has been calculated. Instead of
computing $Q^*_t(s,a)$ and $V^*_t(s)$, proceed as follows:

1. Set $s = 1$ and $α = 1$. 
2. For all $a \in \{α, \dots, m\}$, compute $Q^*_t(s,a)$ as usual. 
3. Compute 
...
Consider a queueing system where $S_t \in \integers_{\ge 0}$ denotes the number of jobs in a buffer, $W_t \in \integers_{\ge 0}$ denotes the number of jobs that arrive in a batch at time~$t$, and $D_t \in \{0, 1\}$ denotes the number of jobs completed at time~$t$. It is assumed that _the arrival process_ $\{W_t\}_{t \ge 1}$ is an i.i.d.\ process with PMF $P_W$, and _the departure process_ $\{D_t\}_{t \ge 1}$ is Bernoulli process with $\PR(D_t = 1) = μ$. 

At the start of each time instance, a controller has to decided whether to accept or reject the next batch of arrivals (knowing only $S_t$ and before seeing the reaization of $W_t$ and $D_t$). We model this by $A_t = 1$ to denote the accept decision and $A_t = 0$ to denote the reject decision. 
...
c. Argue that $σ_t$ is a non-decreasing sequence. 
...
Let $P(a)$ denote the controlled transition matrix and $c_t(s,a)$ denote the per-step cost. To avoid ambiguity, we define the optimal policy as
   $$
     π^*_t(s) = \max\bigl\{ a' \in \arg\min_{a \in \ALPHABET A} Q^*_t(s,a) \bigr\}
   $$
