...
Consider the machine repair example from @exr-machine-repair. We compute the optimal policy for this model when $W \sim \text{Binom}(n,p)$ when $n = 10$, $T = 20$, $h(s) = 2s$, and $λ=20$ for different values of $p$.
...
In the machine replacement problem shown in @fig-machine-replacement, the optimal policy satisfies a qualitative property: _the optimal policy is monotone_, that is if $s < s'$ then $π^*_t(s) < π^*_t(s')$. Intuitively, it makes sense. But how can we establish that this is the case for other choices of problem parameters as well?
:::
...
Consider the example of machine repair presented in @exm-machine-repair. Prove that the optimal policy for that model is weakly increasing.
:::
