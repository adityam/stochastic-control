...
\begin{align*}
  ...
  π^*_t(s) &\in \arg \min_{a \in \S_{\ge 0}} Q^*_t(s,a) 
  \end{align*}
...
\begin{align*}
  ...
  π^*_t(s) &\in \arg \min_{a \in \S_{\ge 0}} \bigl\{ pa + H_t(s + a) \bigr\}.
\end{align*}
...
# # for pkg in ["IJulia", "Revise", "Distributions", "OffsetArrays", "DataFrames", "JSON", "JSON3"]
# #    Pkg.add(pkg)
# # end
...
2. For any convex function $f \colon \reals \to \reals$, 
   let $σ =  \arg \min_{s \in \reals} f(s)$. (If the arg min is not unique, take $σ$ to be the _smallest_ arg min). Then,
...
$$π^*_T(s) \in \arg \min_{a \in \reals_{\ge 0}} f_T(s+a), \quad π^*_T(s) = \max(σ_T - s, 0).
$$
and
...
\begin{align*}
  ...
  π^*_t(s) &\in \arg \min_{a \ge 0} \bigl\{ f_t(s+a) \bigr\}.
\end{align*}

To understand the nature of the optimal policy, we plot the optimal solution for the same choice of parameter values as before. 
...
module LostSales
    ...
    function generate_binomial_demand(n,q,ch,cs,p,T,K)
      ...
      function bellmanUpdate!(v_next, π_next, v; discount=1.0)
          # Assume that v is of size 2L + 1
          # Construct a post-decision state value function
          H = OffsetArray(zeros(2L+1),0:2L)
          # h(z) = E[ h([z-W]^{+}) + γ V([z-W]^{+}) ]
          for z ∈ 0:2L, w ∈ W
              next_s = clamp(z-w, 0, L)
              H[z] += ( h(z-w) + discount * v[next_s] )* pdf(Pw,w)
          end
    ...
    df = DataFrame(time=Int[], state=Int[], value=Float64[], policy=Int[], holding=Float64[])

    Ch = range(start=0.5, step=0.5, stop=4)

    for ch in Ch
      (S,V,π) = generate_binomial_demand(n,q,ch,cs,p,T,0)
      for t in 1:T, s in S
        push!(df, (t, s, V[t][s], π[t][s], ch))
      end
    end
end

ojs_define(LostSales_T = LostSales.T, LostSales_data = LostSales.df)
```

```{ojs}
//| layout-ncol: 2

viewof LostSales_time = Object.assign(Inputs.range([1, LostSales_T], {label: "t", step: 1, value: 1 }), {style: '--label-width:20px'})
viewof LostSales_ch  = Object.assign(Inputs.range([0.5, 5], {label: "ch", step: 0.5, value: 2}), {style: '--label-width:20px'})
```
...
Plot.plot({
  ...
  marks: [
    ...
    Plot.ruleY([0]),
    // Data
    Plot.line(LostSales_data.filter(d => d.time == LostSales_time && d.holding == LostSales_ch), {x:"state", y:"value", curve:"step-after"})
  ]
})
...
Plot.plot({
  ...
  marks: [
    ...
    Plot.ruleY([0]),
    // Data
    Plot.line(LostSales_data.filter(d => d.time == LostSales_time && d.holding == LostSales_ch), {x:"state", y:"policy", curve:"step-after"})
  ]
})
```

As we can see, the optimal policy still appears to have a base-stock structure. This is indeed the case, as shown by the following result.
...
$$
g_t(z) = \begin{cases}
f_{t+1}(σ_{t+1}) - c_s z, & \hbox{if } z < 0 \\
...
The above argument also shows that the slope of $g_t(z)$ (and therefore of $g_t(z) + c_h [z]^{+}$) is $-c_s$ in $(-∞,0)$. Hence, the slope of $f_t(z)$ is strictly negative in $(-∞, 0)$. Thus, $σ_t \not\in (-∞,0)$. 
...
We need a new proof technique to prove this result. The proof for $K=0$ relied on convexity of $H_t$. However, when $K > 0$, $H_t$ is no longer convex. It does, however, satisfy a relaxed property known as $K$-convexity.
...
The definition of $K$-convexity immediately implies the following properties.
...
#### (s-S) policy
The optimal policy given by \\eqref{eq:pi-setup} is called an $(s,S)$- policy, which states that whenever the stock level falls below $σ_t$, the optimal action is to replenish the stock to level $Σ_t$.
:::
...
We now assume that $H_t(z)$ is $K$-convex. Note that part 2 of @lem-K-convex implies that $f_t(z)$ is also $K$-convex. By definition, $Σ_t$ is the minimizer of $f_t(z)$ and $σ_t$ is the smallest value of $z$ for which $f_t(z) = f_t(Σ_t) + K$. Therefore,
...
\end{equation}
(where $\GRAD V^*_t$ denotes the derivative of $V^*_t$).

We consider three cases:

- **Case 1: $σ_t < s \le s + z$.**
In this case, $V^*_t(s)$ is the sum of a $K$-convex and an affine (therefore $0$-convex) function. Hence, by property 2 of @lem-K-convex, it is $K$-convex. Thus, \eqref{eq:verify-setup} is satisfied.
...
\end{align*}
which is nonnegative because $Σ_t$ minimizes $f_t$, so $f_t(s+z) \ge f_t(Σ_t)$.
...
2. Argue that the structure of optimal policies continue to hold, i.e., there exists a sequence $\{σ_t\}$, $σ_t \in \integers$, such that the policy 
   $$
    π^*_t(s) = \begin{cases}
    σ_t - s, & \text{if $s \le σ_t$} \\
    0, & \text{otherwise}.
...
:::{#exr-inventory-uncertain-demand}
Consider inventory management with zero ordering cost (with continuous valued states and actions). Suppose that the demand $W_t$ depends on an exogenous Bernoulli process $\{D_t\}_{t \ge 1}$, where $\PR(D_t = 1) = α$. When $D_t = 0$, the demand $W_t$ is low and is distributed according to PDF $f_0$ and when $D_t = 1$, the demand $W_t$ is high and distributed according to PDF $f_1$. 

At the beginning of each time period, in addition to knowing the stock level $S_t$, the decision maker also observes $D_t$.

a. Write the dynamic program taking $(S_t, D_t)$ as the state.

b. Identify the post-decision state to simplify the dynamic program.

c. Obtain the structure of the optimal policy (you may wish to numerically solve the DP for some choices of the model parameter to guess the structure).
:::

## Notes {-}

Inventory management models with deterministic demand were introduced by @Harris1913. The mathematical model of inventory management considered here was originally proposed by @Arrow1952. The optimality of base-stock policy was established by @Bellman1955. See the notes on [infinite horizon](inventory-management-revisited.html) version of this model to see how to find the threshold in closed form. 

A model where $\S = \reals$ but $\ALPHABET A = \integers_{\ge 0}$ is considered in @Veinott1965. Its generalization with positive ordering cost is considered in @Tsitsiklis1984. 
