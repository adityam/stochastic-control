...
In this section, we assume that the state space $\S$ is equal to $\reals$ (instead of $\integers$). 
...
exists a sequence $\{s^\circ_t\}_{t \ge 1}$ of numbers such that policy given by
$$ π_t(s) = \begin{cases}
   \lceil s_t^\circ - s \rceil, & \text{if } s < s_t^\circ \\
   0, & \text{if } s \ge s_t^\circ.
  \end{cases} $$
is optimal.
:::
--> 
