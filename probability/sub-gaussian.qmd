...
4. An immediate corollary of the previous example is that if $X$ is a random variable with $\EXP[X] = 0$ and $|X| \le b$ a.s., then $X$ is
$b$-sub-Gaussian. This result is also a consequence of [:Hoeffding's Lemma](https://en.wikipedia.org/wiki/Hoeffding's_lemma).
...
For this reason, sometimes it is stated that $X \sim \text{subG}(σ^2)$ when it
...
The Hoeffding inequality can be generalized to Martingales. See @thm-Azuma-MDS.
